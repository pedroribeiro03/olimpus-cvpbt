...
export const config = {
    ...
    aptidaoCollectionId: '6758cd30003d9ef800ae', 
    treinoTecnicoCollectionId: '6820a7e1002a464a83d5', 
    exercicioTecnicoCollectionId: '6758cdb0001f4bf97469',
...
export const getTechnicalExercisesForDate = async (date) => {
  try {
    const currentUser = await getCurrentUser();
    if (!currentUser ) {
      throw new Error("Usuário não autenticado!");
    ...
    const treinoResult = await databases.listDocuments(
      ...
      [
        Query.equal("userFrontend", String(currentUser.$id)),
        Query.greaterThanEqual("data", startOfDay),
        Query.lessThan("data", endOfDay),
      ]
    );
    ...
    const registoResult = await databases.listDocuments(
      ...
      [Query.equal("treinoTecnico", treinoIds)]
    );

    console.log(registoResult.documents)
    // 3. Processamento seguro dos exercícios
    return registoResult.documents
      .map((registo) => {
        // Extrai o objeto do exercício técnico
        const ex = registo.exercicioTecnico || {};

        return {
          nome: ex.nome ?? "Exercício desconhecido",
          idExercicio: ex.$id ?? `temp-${Math.random().toString(36).substr(2, 9)}`,
...
export const getAllTestesIniciais = async (limit = 25, offset = 0) => {
  ...
  // busca múltiplos documentos na coleção de aptidão pelo array de IDs
export const getTiposAptidao = async (id) => {
  const response = await databases.listDocuments(
    config.databaseId,
    config.aptidaoCollectionId,
    [
      Query.equal('$id', id)       // aqui o Appwrite aceita array para equal
    ]
  );
  return response.documents[0].nome;      // cada doc tem { $id, nome, ... }
};

/**
 * Obtém os nomes de todas as aptidões disponíveis.
 */
export const getAptidoes = async () => {
  try {
    ...
    // Mapeia cada documento para o seu nome
    return response.documents.map(doc => doc.nome);
  } catch (error) {
...
export const createTesteInicial = async (nome, instrucoes, tipoTesteId, exercicioGinasioId, unidade_medida) => {
  try {
        const response = await databases.createDocument(
            config.databaseId,
            config.exercicioGinasioCollectionId,
...
export const createExercicioTecnico = async (nome, instrucoes, video, desporto) => {
  try {
        const response = await databases.createDocument(
            config.databaseId,
            config.exercicioTecnicoCollectionId,
            ID.unique(),
            {
                nome,
                instrucoes,
...
export const createTreinoTecnico = async (userId, date) => {
    try {
      const workout = await databases.createDocument(
        config.databaseId,
        config.treinoTecnicoCollectionId,
      ...
      console.error('Erro ao criar sessão de treino técnico:', error);
      throw new Error('Erro ao criar sessão de treino técnico');
    }
  };

export const createTreinoTecnicoRecord = async (exercicioTecnicoId, treinoTecnicoId) => {
  try {
      const record = await databases.createDocument(
        config.databaseId,
        config.registoTreinoCollectionId,
...
export const getAllExerciciosTecnicos = async(limit, offset) =>{
  try {
        const response = await databases.listDocuments(
            config.databaseId,
            config.exercicioTecnicoCollectionId,
...
export const getAllExerciciosTecnicosRugby = async () => {
  try {
    const response = await databases.listDocuments(
      config.databaseId,
      config.exercicioTecnicoCollectionId,
      [
        Query.equal("desporto", "Rugby")
      ]
    );
    return response
  } catch (error) {
...
export const getExerciciosDisponiveis_Rugby = async () => {
  try {
    let allExercises = [];
    let offset = 0;
    const limit = 100;
    ...
    while (true) {
      const response = await databases.listDocuments(
        ...
        config.exercicioTecnicoCollectionId,
        [
          Query.equal("desporto", "Rugby"),    // ou normaliza antes
          Query.limit(limit),
          Query.offset(offset),
        ]
      );
...
export const getAllExerciciosTecnicosFutebol = async() =>{
  try {
        const response = await databases.listDocuments(
            config.databaseId,
            config.exercicioTecnicoCollectionId,
...
const getExerciciosDisponiveis_Futebol = async () => {
  try {
    let allExercises = [];
    offset = 0;
    limit = 100;
    ...
    while (true) {
      const response = await databases.listDocuments(
        config.databaseId,
        config.exercicioTecnicoCollectionId, // ID da coleção de exercícios
        [
          Query.equal("desporto", "Futebol")
        ]
      );
...
export const getExercicioTecnicoById = async (id) => {
    try {
        const exercicio = await databases.listDocuments(
            config.databaseId,
            config.exercicioTecnicoCollectionId,
...
export const updateExercicioTecnico = async (id, nome, instrucoes, video, desporto) => {
    try {
        const response = await databases.updateDocument(
            config.databaseId,
            config.exercicioTecnicoCollectionId,
...
export const deleteExercicioTecnico = async (id) => {
    try {
        await databases.deleteDocument(
            config.databaseId,
            config.exercicioTecnicoCollectionId,
...
//TREINO TÉCNICO
// Função que chama a API da OpenAI para gerar o plano de treino
const callApiToGeneratePlan_Tecnico = async (athleteProfile) => {
  try {
    let listaExs 
    console.log(getExerciciosDisponiveis_Rugby)
    // 1. Obter lista de exercícios disponíveis
    if(athleteProfile.desporto =="rugby"){
      listaExs = await getExerciciosDisponiveis_Rugby();
    }else{
      listaExs = await getExerciciosDisponiveis_Futebol();
    }
    console.log(listaExs)
    // 2. Preparar o prompt a ser enviado à API
    const todayDate = getTodayDate();
    const prompt = `
      Crie um plano de treino técnico para o atleta com as seguintes informações:

      - Desporto: ${athleteProfile.desporto}
      - Posição: ${athleteProfile.posicao}

      Lista de exercícios disponíveis:
      ${listaExs.map(ex => `- ID: ${ex.id}, Nome: ${ex.nome}`).join('\n')}

      Dos próximos 7 dias, deves escolher 3 a 4 dias para criar treinos. 
      Para cada dia que escolhes, deves sugerir 1 ou 2 exercicios por dia.
      Em dias que não há exercicios técnicos simplesmente não deves registar exercicios técnicos
      Assegura-te de que só sugeres exercícios adequados para a posição do atelta
      Não há problema se ele repetir exercícios
        ...
        "treinos": [
          {
            "data": "YYYY-MM-DD",
            "exercicios": [
              {"id_exercicio": "ID1", "nome": "nome do exercicio"},
              {"id_exercicio": "ID2", "nome": "nome do exercicio"}
            ]
          },
          // Continue para os outros dias
        ]
    ...
    const response = await axios.post('https://api.openai.com/v1/chat/completions', {
      ...
      messages: [
        {
          role: 'system',
          content: 'Você é um treinador técnico desportivo. Gere planos de treino personalizados com base nas informações fornecidas.'
        },
        {
          role: 'user',
          content: prompt
...
// Função que faz o tratamento da resposta da API e guarda registos na bd
const saveTrainingPlanToDatabase_Tecnico = async (trainingPlan, userId) => {
  try {
    ...
    for (const treino of trainingPlan.treinos) {
      // 1. Criar o registo de treino_ginasio e AGUARDAR a conclusão
      const treinoTecnico = await databases.createDocument(
        config.databaseId,
        config.treinoTecnicoCollectionId,
        ID.unique(),
        {
          userFrontend: (await getUserById(userId)).$id, 
          data: treino.data
        }
      );
      ...
      for (const exercicio of treino.exercicios) {
        await databases.createDocument(
          config.databaseId,
          config.registoTreinoTecnicoCollectionId, // ID da coleção de exercícios
          ID.unique(), // ID único para cada exercício no treino
          {
            treinoTecnico: treinoTecnico.$id, // Referência ao treino
            exercicioTecnico: exercicio.id_exercicio,
          }
        );
      }
    }
...
export const generateTrainingPlan_Tecnico = async (userId) => {
  try {
    // 1. Obter as informações do atleta
    const athleteProfile = await getAthleteProfile(userId);

    // 2. Chamar a API para gerar o plano de treino
    const trainingPlan = await callApiToGeneratePlan_Tecnico(athleteProfile);

    // 3. Guardar o plano de treino na base de dados
    await saveTrainingPlanToDatabase_Tecnico(trainingPlan, userId);
...
/**
 * Verifica se há pelo menos um exercício de ginásio registrado nos próximos 7 dias.
 * Retorna true se encontrar, false caso contrário.
 */
export const hasGymPlansNext7Days = async () => {
  const hoje = new Date();
  for (let i = 0; i < 7; i++) {
    const d = new Date(hoje);
    d.setDate(hoje.getDate() + i);
    const dateString = d.toISOString().slice(0, 10); // “YYYY-MM-DD”
    const exercises = await getExercisesForDate(dateString);
...
/**
 * Verifica se há pelo menos um exercício técnico registrado nos próximos 7 dias.
 * Retorna true se encontrar, false caso contrário.
 */
export const hasTechPlansNext7Days = async () => {
  const hoje = new Date();
  for (let i = 0; i < 7; i++) {
    const d = new Date(hoje);
    d.setDate(hoje.getDate() + i);
    const dateString = d.toISOString().slice(0, 10); // “YYYY-MM-DD”
    const exercises = await getTechnicalExercisesForDate(dateString);
    if (exercises.length > 0) {
      return true;
    }
  }
...
//NOVAS FUNÇÕES GUI
//Registo testes iniciais

export const getTesteInicialById = async (testeId) => {
  console.log(testeId)
  try {
    const teste = await databases.listDocuments(
      config.databaseId,
      config.testeInicialCollectionId,
      [
        Query.equal("$id", testeId),
      ]
    );
    return teste.documents[0];
  } catch (error) {
...
export const getRegistosByTesteAndUser = async (testeId, userId) => {
  try {
    const response = await databases.listDocuments(
      config.databaseId,
      config.registoTestesIniciaisCollectionId,  // Coleção de registos
      [
        Query.equal('testeinicial', testeId),
        Query.equal('userFrontend', userId)
      ]
    );
    return response.documents; // Retorna os documentos encontrados
  } catch (err) {
...
export const registarResultado = async (testeId, userId, resultado) => {
  try {
    const novoRegisto = await databases.createDocument(
      config.databaseId,
      config.registoTestesIniciaisCollectionId,
      ID.unique(), // Gera um ID único
      {
        testeinicial: testeId,  // Relacionamento com o teste
        userFrontend: userId,   // Relacionamento com o utilizador
        Resultado: resultado,
      }
    );
    return novoRegisto;
  } catch (error) {
    console.error('Erro ao registar resultado:', error.message);
    throw error;
  }
};