...
*/
  ...
  export const deleteTesteInicial = async (id: string) => {
    console.log('[DELETE] Iniciando processo para ID:', id);
    
    try {
      // 1. Verificação robusta de autenticação
      const account = await getAccount();
      if (!account) {
        console.error('[DELETE] Nenhum usuário autenticado');
        throw new Error('Sessão expirada. Faça login novamente.');
      }
  
      // 2. Verificação de backend user (versão reforçada)
      ...
      if (userDoc.total === 0) {
        console.error('[DELETE] Usuário não é backend:', account.$id);
        throw new Error('Acesso restrito a administradores');
      }
  
      // 3. Execução da deleção com timeout
      console.log('[DELETE] Tentando deletar documento...');
      ...
      console.error('[DELETE] Erro completo:', {
...
// Adicione esta função no seu arquivo appwrite.ts
export const verifyConnection = async () => {
  try {
    // Verificação simples de conexão tentando listar documentos
...
export const deleteTreinoTecnico = async (id: string) => {
  try {
    ...
    console.error('Erro ao eliminar treino técnico:', {
      error,
      id,
      time: new Date().toISOString()
    });
...
export {
  client,
  account,
  databases,  // Certifique-se que está exportando
  // ... outras exportações
};
