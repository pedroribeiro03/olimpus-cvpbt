  ...
  export const getAllUsers = async (limit = 25, offset = 0) => {
    try {
      const response = await databases.listDocuments(
        ...
        config.user_frontEndCollectionId,
        [], // Filtros (opcional)
        limit, // Limite de documentos por página
        offset // Deslocamento (página atual * limite)
      );
...
export const getAllExercises = async (limit = 25, offset = 0) => {
  ...
  export const getGruposMusculares = async () => {
    try {
        ...
        return response.documents.map((doc) => ({
            label: doc.nome, // Nome do grupo muscular
            value: doc.$id, // ID do grupo muscular
        }));
    } catch (error) {
  ...
  export const createExercise = async (nome, video, instrucoes, grupoMuscular1, grupoMuscular2) => {
    try {
        const response = await databases.createDocument(
            config.databaseId, // ID do banco de dados
            config.exercicioGinasioCollectionId, // ID da coleção exercicioGinasio
            ID.unique(), // ID único gerado automaticamente
            {
                nome,
                video,
                instrucoes,
        ...
        return response; // Retorna o documento criado
    } catch (error) {
        console.error('Erro ao criar exercício:', error);
...
export const updateExercise = async (id, nome, video, instrucoes, grupoMuscular1, grupoMuscular2) => {
  try {
      ...
      return response; // Retorna o documento atualizado
  } catch (error) {
...
/**
 * Função para buscar um exercício pelo ID (para utilizadores de backend).
 * @param {string} id - ID do exercício.
 * @returns {Promise} - Retorna uma promessa com os dados do exercício.
 */
export const getExerciseByIdBackend = async (id) => {
  try {
    ...
    return response.documents; // Retorna um array de documentos
  } catch (error) {
