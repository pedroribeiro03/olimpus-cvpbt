...
*/
  ...
  export const deleteTesteInicial = async (id: string) => {
    try {
      // Cria um novo cliente para garantir autenticação fresca
      const freshClient = new Client()
        .setEndpoint(config.endpoint)
        .setProject(config.projectId);
      
      const freshAccount = new Account(freshClient);
      const freshDatabases = new Databases(freshClient);
  
      // Verifica a sessão atual
      const currentSession = await freshAccount.getSession('current');
      if (!currentSession) {
        throw new Error('Sessão expirada. Por favor, faça login novamente.');
      }
  
      // Verifica se é admin/backend
      const isAdmin = await freshDatabases.listDocuments(
        config.databaseId,
        config.user_backEndCollectionId,
        [Query.equal('IDUtilizador', currentSession.userId)]
      );
  
      if (isAdmin.total === 0) {
        throw new Error('Apenas administradores podem deletar testes');
      }
      
      // Executa a deleção
      await freshDatabases.deleteDocument(
        config.databaseId,
        config.testeInicialCollectionId,
        id
      );
      ...
      throw new Error(error.message || 'Falha ao deletar teste');
    }
  };
